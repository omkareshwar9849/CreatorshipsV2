...
const CreatorForm = (props) => {
  ...
  return (
    <div className="flex min-h-[100dvh] bg-background" style={{marginBottom:'20px'}}>
          ...
          <div className="rounded-lg border bg-card text-card-foreground shadow-sm" data-v0-t="card" style={{backgroundColor:'#EBD96B'}}>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="space-y-2 text-center">
                <h1 className="text-3xl font-bold">{formData._id ? "Update Your Profile" : "Register as a Creator"}</h1>
                <p className="text-muted-foreground">{formData._id ? "Update your profile details below." : "Enter your details to continue."}</p>
              </div>
                ...
                {Object.keys(formData).map((key) => (
                  key !== "_id" && key !== "__v" && key !== "type" && key !== "user" && key !== "createdAt" && key !== "updatedAt" && (
                    <div key={key} className="grid gap-2">
                      ...
                      {key === "previousWorkExperience" || key === "partnershipGoals" || key === "skills" ? (
                        <textarea
